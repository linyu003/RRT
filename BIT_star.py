...
class Environment:
    ...
    def parse_rectangle(self, name, description) -> Polygon:
...
class BITStar:
    def __init__(self, environment: Environment, start: tuple[int], goal:tuple[int], 
                 bounds: tuple[int, int, int, int], 
                 maxIter: int =50000, plot_flag: bool =True):
    ...
    def plan(self, pathLengthLimit: int):
...
if __name__ == '__main__':
    bounds:tuple[int,int,int,int] = (-120, -120, 120, 120)
    maxIter:int = 100000

    # RRT_circle_enviroment
    # best path length: 196
    pathLengthLimit:int = 200
    environment:Environment = Environment(None,bounds)
    # A circle in the center
    environment.add_obstacles([Point(0,0).buffer(40)])
    start:tuple[int] = (0, 90)
    goal:tuple[int] = (0,-90)
    ...
    plot_planning_process:bool = True
